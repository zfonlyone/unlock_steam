"""
启用固定清单 - 将 Lua 中的 --setManifestid 改为 setManifestid
支持直接调用和命令行两种方式

直接调用:
    from tools.enable_manifest import run_enable
    result = run_enable(target_dir, progress_callback)

命令行:
    python enable_manifest.py [目录]
"""
import os
import sys
import re
...
def enable_in_file(file_path: str) -> tuple:
    """
...
def run_enable(target_dir: str, progress_callback=None) -> dict:
    """
    运行启用并返回结果
    ...
    for i, file_path in enumerate(lua_files):
        result = enable_in_file(file_path)
        if result[1]:  # 有修改
            modified_count += 1
    ...
    # 生成结果
    if modified_count > 0:
        log(f"已启用 {modified_count} 个文件的固定清单:")
