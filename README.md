...
- **配置对话框**：首次使用时自动提示配置必要路径
- **打包功能**：支持打包成单个EXE文件便于分发
- **防篡改保护**：内置程序完整性验证，防止程序被恶意修改
- **版本管理**：自动版本追踪和构建信息记录
- **项目信息管理**：统一管理程序名称、版本号等信息

## 新增功能（v1.1.0）

- **程序完整性验证**：启动时自动检测程序是否被篡改，确保安全
- **版本信息显示**：在窗口标题和关于对话框中显示版本信息
- **构建信息记录**：自动记录构建日期和版本信息，便于跟踪
- **关于对话框**：通过菜单"帮助 > 关于"查看程序详细信息
- **更新逻辑重构**：根据appid更新游戏数据，仅更新需要更新的字段
- **剪贴板复制修复**：修复了复制游戏名称到剪贴板的功能
...
7. **安全机制**：防篡改验证确保程序完整性和用户安全

## 安装与使用

### 重要安全提示

本程序包含完整性验证机制，如果检测到程序文件被篡改，将拒绝启动并显示警告。因此：
- **请勿修改**程序文件或资源
- **请从官方渠道**获取程序，避免使用第三方修改版本
- 如果您收到篡改警告，请重新下载官方版本
...
4. **steam_names_cache.json**：缓存从Steam API获取的游戏名称

## 安全性说明

应用程序集成了防篡改保护机制：

1. **完整性验证**：启动时验证程序代码和信息的完整性
2. **签名机制**：使用HMAC-SHA256算法对程序关键信息进行签名
3. **运行时检测**：检测运行时可能的篡改尝试
4. **安全警告**：如果检测到篡改，会显示警告并安全退出
...
本项目基于GNU通用公共许可证v3 (GPL-3.0)开源。使用本软件时，您必须遵守以下条款：

1. 您可以自由使用、复制、分发和修改本软件
2. 如果您修改了本软件，必须同样以GPL-3.0许可证发布
3. 您必须保留原始版权声明和许可信息

完整的许可证文本请参阅项目中的`LICENSE`文件或访问[GNU GPL v3](https://www.gnu.org/licenses/gpl-3.0.html)。

Copyright © 2023-2024 zfonlyone. All rights reserved.